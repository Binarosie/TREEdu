...
public class QuizAttemptServiceImpl implements iQuizAttemptService {
        ...
        public QuizAttemptResponse submitQuiz(String quizId, SubmitQuizRequest request, String userId) {
                ...
                // Khai báo các biến Gamification ra ngoài khối block if để map vào Object trả
                // về cuối hàm
                int xpGained = 0;
                ...
                if (user instanceof Member member) {
                        ...
                        // Cập nhật thông tin Chuỗi ngày học (Nhớ xóa bỏ dòng
                        // userRepository.save(member) bên trong hàm updateStreak nhé ông)
                        ...
                        // Tăng tiến trình nhiệm vụ "Làm Quiz bất kỳ" lên 1
                        missionService.fireMissionEvent(userId,
                                        vn.hcmute.edu.materialsservice.Enum.EMissionType.DO_QUIZ_ANY, 1);

                        // Tăng tiến trình nhiệm vụ "Tích lũy XP trong ngày" (nếu có kiếm được XP)
                        if (xpGained > 0) {
                                missionService.fireMissionEvent(userId,
                                                vn.hcmute.edu.materialsservice.Enum.EMissionType.EARN_DAILY_XP,
                                                xpGained);
                        }
                        ...
                        if (correctCount == quiz.getQuestionCount() && quiz.getQuestionCount() > 0) {
                                ...
                                missionService.fireMissionEvent(userId,
                                                vn.hcmute.edu.materialsservice.Enum.EMissionType.GET_PERFECT_SCORE, 1);
                        }

                        // ========================================================
                }

                log.info("Quiz submitted: attemptId={}, score={}/{}", attempt.getId(), correctCount,
        ...
        public List<QuizAttemptResponse> getUserAttemptHistory(String userId) {
                ...
                List<QuizAttempt> attempts = attemptRepository
                                .findByUserIdAndSubmittedTrueOrderBySubmittedAtDesc(userId);
        ...
        public List<QuizAttemptResponse> getUserAttemptsByQuiz(String quizId, String userId) {
                ...
                List<QuizAttempt> attempts = attemptRepository
                                .findByQuizIdAndUserIdAndSubmittedTrueOrderBySubmittedAtDesc(
                                                quizId, userId);
        ...
        public QuizAttemptResponse getAttemptDetail(String attemptId, String userId) {
                ...
                return QuizAttemptResponse.builder()
                                ...
                                .results(results)
                                .submittedAt(attempt.getSubmittedAt())
                                // Điền các giá trị mặc định để tránh lỗi vỡ cấu trúc DTO khi xem chi tiết bài
                                // cũ
                                .xpGained(0)
                                .leveledUp(false)
                                .currentLevel(1)
        ...
        public QuizDashboardResponse getAdminDashboardStats() {
                ...
                // Cố định múi giờ Việt Nam, tránh lỗi lệch múi giờ UTC gây sai số liệu khi
                // deploy Cloud
                ...
                List<QuizDashboardResponse.TopQuizStat> topQuizzes = topRaw.stream().map(item -> {
                        ...
                        if (currentQuizId != null) {
                                ...
                                log.warn(" Found aggregation result with null quizId");
                        }
