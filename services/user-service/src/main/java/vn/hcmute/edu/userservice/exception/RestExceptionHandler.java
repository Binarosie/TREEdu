package vn.hcmute.edu.userservice.exception;
...
import vn.hcmute.edu.userservice.dto.response.ApiError;
...
public class RestExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleDuplicateKey(Exception ex) {
        ...
        if (ex.getCause() != null && ex.getCause().getMessage() != null) {
            ...
            if (causeMsg.contains("phone")) {
                message = ValidationMessages.MEMBER_PHONENUMBER_DUPLICATE;
            } else if (causeMsg.contains("email")) {
                message = ValidationMessages.MEMBER_EMAIL_DUPLICATE;
    ...
    private String getFieldErrorMessage(FieldError fieldError) {
        ...
        switch (code.toLowerCase()) {
            ...
            case "email":
                return ValidationMessages.MEMBER_EMAIL_INVALID;
            default:
                return String.format("%s: %s", fieldName, fieldError.getDefaultMessage());
        }
    }
    ...
    private String mapRequiredFieldError(String fieldName) {
        switch (fieldName.toLowerCase()) {
            case "fullname":
                return ValidationMessages.MEMBER_FULLNAME_REQUIRED;
            case "email":
                return ValidationMessages.MEMBER_EMAIL_REQUIRED;
            case "phonenumber":
                return ValidationMessages.MEMBER_PHONENUMBER_REQUIRED;
            case "birthday":
                return ValidationMessages.MEMBER_BIRTHDAY_REQUIRED;
            case "address":
                return ValidationMessages.MEMBER_ADDRESS_REQUIRED;
            default:
    ...
    private String mapPatternError(String fieldName, String rejectedValue) {
        switch (fieldName.toLowerCase()) {
            case "phonenumber":
                return ValidationMessages.MEMBER_PHONENUMBER_INVALID;
            case "email":
                return ValidationMessages.MEMBER_EMAIL_INVALID;
            default:
    ...
    private String mapSizeError(String fieldName, String rejectedValue) {
        switch (fieldName.toLowerCase()) {
            case "fullname":
                return ValidationMessages.MEMBER_FULLNAME_SIZE;
            default:
